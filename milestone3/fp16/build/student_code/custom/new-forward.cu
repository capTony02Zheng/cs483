...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    if (use_normal == 1) base_pro(host_output, host_input, host_mask, device_output_ptr, device_input_ptr, device_mask_ptr,B,M, C, H, W,  K, S);
    if (use_fp16 == 1) fp16_pro(host_output, host_input, host_mask, device_output_ptr, device_input_ptr, device_mask_ptr,B,M, C, H, W,  K, S);
    if (use_unroll == 1) m_unroll_pro(host_output, host_input, host_mask, device_output_ptr, device_input_ptr, device_mask_ptr,B,M, C, H, W,  K, S);
    if (use_stream == 1) stream_all(host_output, host_input, host_mask, device_output_ptr, device_input_ptr, device_mask_ptr,B,M, C, H, W,  K, S); // this funtion handle all
...
{   
    ...
    dim3 dimBlock(TILE_WIDTH, TILE_WIDTH, 1);
    if (use_normal == 1)conv_forward_kernel<<<dimGrid, dimBlock>>>(device_output, device_input, device_mask, B, M, C, H, W, K, S); // normal
    if (use_fp16 == 1)conv_forward_kernel_fp16<<<dimGrid, dimBlock>>>(device_output, device_input, device_mask, B, M, C, H, W, K, S); // fp16
...
__host__ void GPUInterface::conv_forward_gpu_epilog(float *host_output, float *device_output, float *device_input, float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    const int W_out = (W - K)/S + 1;
    // normal
    if (use_fp16 == 0) cudaMemcpy(host_output, device_output, (B * M * H_out * W_out)*sizeof(float), cudaMemcpyDeviceToHost); // comment when use fp16
